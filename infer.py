...
class Infer(InferBase):
    ...
    def construct_input(sample_num, sample_dim):
        return StandardLogistic.sample([sample_num, sample_dim])
    ...
    def generate(self, epoch, step):
        ...
        with tf.Session() as sess:
            ...
            flow_out = sess.run([self.flow_out], feed_dict={self.inputs: net_input})
            ...
            torchvision.utils.save_image(torchvision.utils.make_grid(flow_out),
                                         './samples/' + TrainBasic.filename + 'epoch%d_' % epoch + 'step%d.png' % step)
